...
let
  cfg = config.services.terraria2;
  opt = options.services.terraria2;
  ...
  startScript = pkgs.writeShellScript "terraria-start" ''
    ${tmuxCmd} new -d ${pkgs.terraria-server}/bin/TerrariaServer -config ${configFile}
    chmod 660 ${lib.escapeShellArg cfg.dataDir}/terraria.sock
  '';
...
in
{
  imports = [
    ...
    ] "Configure services.terraria2.autocreate instead")
  ];
  options = {
    services.terraria2 = {
      enable = lib.mkOption {
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = [
      "d '${cfg.dataDir}' 0770 terraria terraria - -"
    ];
